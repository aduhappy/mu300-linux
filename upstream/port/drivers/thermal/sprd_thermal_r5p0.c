...
static int sprd_thm_temp_read(struct thermal_zone_device *tz, int *temp)
{
...
static int sprd_thm_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(thm->regbase))
		return PTR_ERR(thm->regbase);
	...
	for_each_child_of_node(np, sen_child) {
		...
		sen->thmzone_dev =
		    devm_thermal_of_zone_register(sen->dev, sen->id,
						  sen, &sprd_thm_ops);
		if (IS_ERR(sen->thmzone_dev)) {
			/* a sensor without a thermal zone is not fatal */
			dev_warn(&pdev->dev, "no thermal zone for sensor %d (%ld)\n",
				 sen->id, PTR_ERR(sen->thmzone_dev));
			continue;
		}
