...
static int sprd_sc_comp_set_rate(struct clk_hw *hw, unsigned long rate,
			     unsigned long parent_rate)
{
	struct sprd_comp *cc = hw_to_sprd_comp(hw);

	return sprd_sc_div_helper_set_rate(&cc->common, &cc->div,
				       rate, parent_rate);
}

static int sprd_sc_comp_set_parent(struct clk_hw *hw, u8 index)
{
	struct sprd_comp *cc = hw_to_sprd_comp(hw);

	return sprd_sc_mux_helper_set_parent(&cc->common, &cc->mux, index);
}
...
const struct clk_ops sprd_sc_comp_ops = {
	.get_parent	= sprd_comp_get_parent,
	.set_parent	= sprd_sc_comp_set_parent,

	.determine_rate	= sprd_comp_determine_rate,
	.recalc_rate	= sprd_comp_recalc_rate,
	.set_rate	= sprd_sc_comp_set_rate,
};
EXPORT_SYMBOL_GPL(sprd_sc_comp_ops);
