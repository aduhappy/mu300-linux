...
void sprd_pcie_restore_msi_ctrls(struct dw_pcie *pci)
{
	...
	/* the MSI target address is reprogrammed by dw_pcie_setup_rc() */
...
void sprd_pcie_teardown_msi_irq(unsigned int irq)
{
	...
	while (data && data->parent_data)
		data = data->parent_data;	/* bottom (DWC) domain, chip_data is the root port */
	if (!data)
		return;
