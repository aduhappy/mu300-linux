...
/*
 * MU300: on the mainline kernel the secure world's IPI_TRUSTY_SMC (SGI 13) never reaches Linux, so Trusty never
 * gets CPU time to run a TA (e.g. modem boot requests block forever). Give it time periodically instead.
 */
static unsigned int nop_poll_ms;	/* superseded by trusty-core nop_poll_ms */
module_param(nop_poll_ms, uint, 0644);

static void trusty_irq_nop_poll(struct work_struct *work)
{
	struct trusty_irq_state *is = container_of(to_delayed_work(work), struct trusty_irq_state, nop_poll);

	if (!nop_poll_ms)
		return;
	trusty_enqueue_nop(is->trusty_dev, NULL);
	/* this Trusty only takes NOPs from CPU 0 and 1 (SMC_AVAILABLE_CPUS) */
	queue_delayed_work_on(0, system_wq, &is->nop_poll, msecs_to_jiffies(nop_poll_ms));
}
...
static int trusty_irq_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&is->nop_poll, trusty_irq_nop_poll);
	if (nop_poll_ms) {
		dev_info(&pdev->dev, "polling trusty with a NOP every %u ms\n", nop_poll_ms);
		/* this Trusty only takes NOPs from CPU 0 and 1 (SMC_AVAILABLE_CPUS) */
	queue_delayed_work_on(0, system_wq, &is->nop_poll, msecs_to_jiffies(nop_poll_ms));
	}
...
static void trusty_irq_remove(struct platform_device *pdev)
{
	...
	cancel_delayed_work_sync(&is->nop_poll);
